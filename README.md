...
20業種 × 12テーマ = **1巡240記事**。1巡後は同じ業種×テーマを第2巡、第3巡として追加生成する（config.jsonの上限設定: 1000記事）。
...
- `pick_next_topic()`: 業種バランスを考慮して次のトピックを選択（記事数が少ない業種を優先）。全組み合わせ生成後は第2巡以降として同じ業種×テーマの別記事を生成
...
- **増分生成**: 生成済みトピックを `history.json` で管理し、1巡目の重複を防止。全組み合わせ生成後は巡回番号付きで追加生成
