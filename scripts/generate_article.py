...
- ペルソナの紹介が終わった直後に、必ず以下の注記を入れる:
  「※本記事の事例は、{industry}でよくあるお悩みをもとに構成したフィクションです。登場人物名・店舗名・会社名は仮名であり、実在の個人・団体とは関係ありません。」

【記事構成の要件（AIO/SEO最適化）】
1. タイトルはh1（# {title}）で始める
2. 4000〜5000文字（しっかり読み応えのある分量）
3. 冒頭リード文（4〜5行）: ペルソナの「ある1日」を描写し、読者に「自分のことだ」と思わせる。時事ネタにも軽く触れる
4. h2見出しを5〜6個。**見出しは疑問形・課題形**にする（AIが回答として引用しやすくなる）:
   - 「{industry}の{topic}、なぜこんなに大変なのか？」（ペルソナの1日と困りごと深掘り）
   - 「{industry}で{topic}が放置されがちな3つの理由」（業界構造・時間・心理の壁）
   - 「{topic}をAIで解決する方法（ステップ解説）」（解決策1。手順を番号付きで）
   - 「もうひとつのAI活用法：〇〇との組み合わせ」（解決策2）
   - 「導入前後でどう変わった？数字で見るビフォーアフター」
   - 「まとめ：ペルソナからのひとこと」
5. 各セクション内でh3も使って読みやすくする
6. **解決策のステップは必ず番号付きリスト（1. 2. 3.）で書く**（HowToスキーマ抽出用）
7. **冒頭リード文の直後に、記事の結論を1〜2文で先に示す**（AIが要約として引用しやすい。例：「結論から言えば、{industry}の{topic}はChatGPTと無料ツールの組み合わせで月○時間削減できます」）
...
【自社サービスの自然な紹介（該当する場合のみ）】
以下の自社サービスがテーマに関連する場合、ペルソナが「実際に使った／見つけた」として自然に1〜2文で言及する:
- メニュー表・チラシ関連 → 「MenuPrint（メニュープリント）」: スマホだけでプロ品質のメニュー表作成、コンビニ印刷可、月額980円〜
- 集客・口コミ・マーケティング関連 → 「AIOスコア診断」: Claude/ChatGPT/GeminiにAIで自分の店が紹介されるか無料診断
- 旅館・飲食店のインバウンド・多言語関連 → 「おもてなしQRメーカー」: 日本語入力だけで15言語AI音声動画をQRコード化、1分30円
→ 押し売りではなく「こんなサービスも見つけた」程度の軽い言及でOK

【SaaSツール紹介の組み込み】
記事の解決策セクション内で、ペルソナが実際に使ったツールとして以下を自然に紹介する:
- 経理系: freee会計（クラウド会計、無料プランあり）
- 予約系: STORES予約（予約管理、無料プランあり）
- 集客系: LINE公式アカウント（メッセージ配信、月200通無料）
- SNS系: Canva（画像作成、無料版あり）/ Canva Pro（30日無料トライアル）
- 顧客管理系: HubSpot CRM（永久無料プラン）
→ テーマに合うものを1〜2つ、「ペルソナが実際に導入した」ストーリーとして具体的に紹介する
→ ツール名の直後に「<!-- TOOL:ツール名 -->」タグを入れる（例: <!-- TOOL:freee会計 -->）
